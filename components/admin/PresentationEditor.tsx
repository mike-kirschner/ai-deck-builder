...
export default function PresentationEditor({ presentationId }: PresentationEditorProps) {
  ...
  async function fetchPresentation() {
    try {
      ...
      showStatus('Failed to load presentation', 'error');
    }
  }
  ...
  async function fetchPages() {
    setLoading(true);
    try {
      const response = await fetch(`/api/presentations/${presentationId}/pages`);
      if (!response.ok) {
  ...
  async function handleTitleChange(pageId: string, newTitle: string) {
    try {
      const response = await fetch(`/api/presentations/${presentationId}/pages/${pageId}`, {
        method: 'PATCH',
  ...
  async function handleOrderChange(newOrder: PageRecord[]) {
    setSaving(true);
    try {
      const response = await fetch(`/api/presentations/${presentationId}/pages/order`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function moveSectionDown(section: string | undefined) {
    ...
    if (lastIndex >= sortedPages.length - 1) return;
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold mt-2 text-gray-100">
            {presentation?.title || 'Loading...'}
          </h1>
          <p className="text-gray-400 mt-2">
            Manage slide order, titles, visibility, and sections for this presentation.
          </p>
        </div>
...
// Import the PageRow and SectionHeader components from AdminInterface
// For now, we'll duplicate them here to keep it self-contained
